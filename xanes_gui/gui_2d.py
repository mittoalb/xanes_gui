...
def zp_motor_from_cal(energy_eV, cal_points):
    ...
    `cal_points` is a list of [E_eV, motor_mm] pairs (e.g. captured by the
    TXM Optics Calculator's calibration dialog). Uses cubic interpolation
    when ≥4 points are available and scipy is present, else linear. Returns
...
class ScanWorker(QThread):
    ...
    def _run_qgmax(self):
        ...
        ack check lets us detect "QGMax isn't listening" in seconds instead
        of the full timeout."""
        ...
        try:
            ...
            return
        self.log.emit(f"    QGMax: request written (ts={req_ts:.3f}), waiting…")
        ...
        while time.time() - t0 < timeout:
            ...
            if float(state.get("last_completed_ts", 0) or 0) >= req_ts:
                ...
                return
            if not acknowledged:
                if float(state.get("started_ts", 0) or 0) >= req_ts:
                    acknowledged = True
                    self.log.emit("    QGMax: ack received, optimizing…")
                elif time.time() - t0 >= ack_timeout:
                    self.log.emit(
                        f"    QGMax: no ack within {ack_timeout:.0f}s — the "
                        f"QGMax dialog in pystream is probably not open, or "
                        f"pystream is running an older build. "
                        f"Skipping optimization for this step."
                    )
                    return
            time.sleep(0.5)
